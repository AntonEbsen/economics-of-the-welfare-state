...
| `robustness.py` | baseline + regime-interaction tables, per-regime marginal-effects tables (with proper linear-combination SEs), post-communist exclusion robustness, stepwise specs, subperiod splits, heterogeneity by welfare regime, feedback (reverse-causality) regressions, KOF sub-component regressions, residual-based Pesaran CD test, SE-estimator sensitivity (one-way entity vs. two-way vs. Driscoll-Kraay) |
