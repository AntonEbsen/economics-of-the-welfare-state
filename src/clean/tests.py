...
def test_stationarity(
    ...
    for var in variables:
        ...
        for country in df[id_var].unique():
            series = (
                df[df[id_var] == country]
                .sort_values(time_var)[var]
                .dropna()
            )
            if len(series) < 10:
                continue
    ...
    if test == 'all':
        ...
        if expected.issubset(pivot.columns):
            pivot['consensus'] = pivot.apply(
                lambda r: 'I(0) ✅' if (
                    r['interpretation_ADF']  == 'Stationary' and
                    r['interpretation_PP']   == 'Stationary' and
                    r['interpretation_KPSS'] == 'Stationary'
                ) else 'I(1) ⚠️ ',
                axis=1,
            )
        ...
        if verbose:
            ...
            cols = ['variable'] + [c for c in pivot.columns if 'interpretation' in c or c == 'consensus']
