...
def generate_methods_section(
    ...
    """
    from .metadata import VARIABLE_METADATA, get_variable_info
    ...
    if include_summary_stats:
        ...
        sections.append("*(Summary statistics table would be inserted here using `generate_summary_stats()`)*\n")
...
def generate_data_appendix(df: pd.DataFrame, output_path: str = None) -> str:
    ...
    for var in sorted(df.columns):
        if var not in ['iso3', 'year']:
            info = get_variable_info(var)
            if 'error' not in info:
                rows.append(f"| {var} | {info['label']} | {info['source']} | {info['unit']} | {info['description']} |")
