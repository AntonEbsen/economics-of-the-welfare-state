...
def plot_correlation_matrix(
    df: pd.DataFrame, variables: list[str] = None, figsize: tuple = (10, 8), save_path: str = None
):
    """
...
def plot_country_coverage(
    df: pd.DataFrame, variable: str = None, figsize: tuple = (14, 10), save_path: str = None
):
    """
    ...
    if variable:
        pivot = df.pivot_table(
            index="iso3", columns="year", values=variable, aggfunc="count"
        ).fillna(0)
    else:
    ...
    plt.figure(figsize=figsize)
    sns.heatmap(pivot, cmap="YlGnBu", cbar_kws={"label": "Data available"}, linewidths=0.5)
