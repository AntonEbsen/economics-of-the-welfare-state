...
# Find ALL code cells containing the broken f-string print line
for i, cell in enumerate(nb["cells"]):
    if cell["cell_type"] == "code":
        src = "".join(cell["source"])
        if "{'='*20}" in src or "int_conservative" in src:
            print(f"Found target at cell {i}")
            nb["cells"][i]["source"] = [
